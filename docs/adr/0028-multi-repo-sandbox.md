...
Users want one agent in one sandbox to manage multiple independent git repos — with worktrees, branches, and commit extraction for each.

## Decision

Sandcastle does not support managing multiple independent git repos (worktrees, branches, commit extraction) within a single sandbox session today.
...
- Single-repo assumptions remain throughout the factory, lifecycle, and sync pipeline until a dedicated multi-repo API ships.
- Secondary repos can be bind-mounted via `mounts` on docker/podman providers, without Sandcastle-managed worktree or commit semantics.
