...
Requests recur to add dedicated `docker()` options for individual Docker run features — custom `context`, `ports` publishing, GPU reservations, and similar one-off flags.

## Decision

Sandcastle does not add a dedicated `docker()` option for every Docker run feature.
...
The built-in `docker()` provider stays focused on the common path, and full-fidelity Docker control lives behind the compose provider or a custom provider rather than an ever-growing option list.
...
- `docker()` options remain limited to the common path; exotic Docker configuration uses `dockerCompose()` or a custom provider.
- See ADR 0023 for the general policy on growing built-in sandbox providers.
